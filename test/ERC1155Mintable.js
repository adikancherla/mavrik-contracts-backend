...
async function testSafeBatchTransferFrom(
    ...
    for (let id of ids) {
        preBalanceFrom.push(
            new BigNumber(await mainContract.balanceOf(from, id))
        );
...
contract("ERC1155Mintable - tests all core 1155 functionality.", accounts => {
    ...
    it("Create initial items", async () => {
        ...
        tx = await mainContract.create(swordQuantity, swordUri, {
            from: user1
        });
    ...
    it("safeBatchTransferFrom ok with valid reciever contract reply", async () => {
        await receiverContract.setShouldReject(false);
        await mainContract.safeBatchTransferFrom(
            user1,
            receiverContract.address,
            idSet,
            quantities,
    ...
    it("safeBatchTransferFrom by operator with enough balance", async () => {
        ...
        tx = await mainContract.setApprovalForAll(user3, false, {
            from: user1
        });
    ...
    it("ERC1165 - returns false on non-supported insterface", async () => {
        ...
        assert(
            (await mainContract.supportsInterface(someRandomInterface)) ===
                false
        );
    });
    ...
    it("ERC1165 - supportsInterface: ERC165", async () => {
        ...
        assert(
            (await mainContract.supportsInterface(erc165interface)) === true
        );
    });
    ...
    it("ERC1165 - supportsInterface: ERC1155", async () => {
        ...
        assert(
            (await mainContract.supportsInterface(erc1155interface)) === true
        );
    });
    ...
    it("setURI only callable by minter (implementation specific)", async () => {
        ...
        await expectThrow(
            mainContract.setURI(newHammerUri, hammerId, { from: user2 })
        );
    });
    ...
    it("mint (implementation specific) - callable only from initial minter ()", async () => {
        await expectThrow(
            mainContract.mint(hammerId, [user3], [1], { from: user2 })
        );
    });
